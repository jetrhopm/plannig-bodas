...
@php
    ...
    $config = ['basePath'=>$basePath,'csrf'=>csrf_token(),'wedding'=>$navigation['wedding'],'items'=>collect($navigation['items'])->map(fn ($item) => [...$item,'icon'=>$icons[$item['label']] ?? 'dossier','href'=>$basePath.'/bodas/'.$navigation['wedding']['id'].$paths[$item['route']],'active'=>$activeRoute ? $item['route'] === $activeRoute : request()->routeIs($item['route'])])->values()];
@endphp
