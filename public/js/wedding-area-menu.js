(() => {
    ...
    class WeddingAreaMenu extends HTMLElement {
        ...
        render() {
            ...
            const displayDate = event.date ? new Intl.DateTimeFormat('es-MX', { weekday: 'long', day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(`${event.date}T12:00:00`)) : 'Fecha pendiente';
            ...
            </style><section class="wedding-area-menu"><section class="area-hero"><p>CENTRO DE TRABAJO</p><h1>${esc(config.wedding?.name)}</h1><div class="area-copy">Organiza, planifica y haz realidad momentos inolvidables.</div><aside class="area-date"><small>${esc(displayDate)}</small><b>${esc(event.venue || 'Lugar pendiente')}</b></aside></section><nav class="area-nav" aria-label="Áreas de trabajo"><div class="area-grid">${cards}</div></nav></section>`;
        }
    }
